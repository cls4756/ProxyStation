...
var SECRET_KEY = "YOUR_SUPER_SECRET_KEY";
var TIME_WINDOW_MS = 3e4;
var app = new Hono2();
app.all("/api/tcp", async (c) => {
  const k = c.req.query("k");
  const h = c.req.query("h");
  const p = c.req.query("p");
  if (!k || !h || !p) {
    return c.text("Missing required parameters: k, h, p", 400);
  }
  const upgradeHeader = c.req.header("Upgrade");
  if (!upgradeHeader || upgradeHeader.toLowerCase() !== "websocket") {
    return c.text("Expected Upgrade: websocket", 426);
  }
  const isAuthorized = await verifyTimeBasedKey(k, SECRET_KEY);
  if (!isAuthorized) {
    return c.text("Forbidden: Invalid or Expired Key", 403);
  }
  ...
  let tcpSocket;
  try {
    tcpSocket = connect({ hostname, port: portMsg });
    ...
    return c.text(`TCP Connect Error: ${error.message || "Unknown"}`, 500);
  }
  ...
  webSocketToTcpStream.pipeTo(tcpSocket.writable).catch(() => {
  });
  tcpSocket.readable.pipeTo(tcpToWebSocketStream).catch(() => {
  });
  return new Response(null, {
    status: 101,
    // @ts-ignore (绕过原生 Response 类型不支持 webSocket 属性的限制)
    webSocket: client
  });
});
...
__name(verifyTimeBasedKey, "verifyTimeBasedKey");
var src_default = app;
export {
